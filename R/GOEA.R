...
do.GOEA = function(cls,univ) {
  univ = subset(select(Mus.musculus, keys=univ, columns=c("ENTREZID"), keytype="SYMBOL"),!is.na(ENTREZID))
  geneLists = vector(mode = "list",length = length(cls))
  names(geneLists) = names(cls)
  for (i in 1:length(geneLists)) {
    ann <- subset(select(Mus.musculus, keys=cls[[i]], columns=c("ENTREZID"), keytype="SYMBOL"),!is.na(ENTREZID))
    geneLists[[i]] <- list(genes=unique(ann$ENTREZID), universe=unique(univ$ENTREZID), annotation='org.Mm.eg.db')
  }
  #KEGG is not working!,"CC","MF"
...
get.table = function(enrichLists,proc,i,th=0.05,th_fdr = 1) {
  ...
  if (proc == "KEGG") {
    ...
    desc = select(Mus.musculus, keys=t$ID, columns=c("TERM"), keytype="GO")
...
# universe <- c("Cst3","Ubb","Dbi","Malat1","Custom_eCFP1_gene","Ppia")
# gene_set1 <- c("Cst3","Ubb","Dbi")
# gene_set2 <- c("Custom_eCFP1_gene","Ppia")
...
# head(subset(results[["KEGG"]],groupID=="set1"))


# which(results[["BP"]]$FDR<0.7)
